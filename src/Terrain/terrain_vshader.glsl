...
void main() {
    ...
    if (isReflected < 0.5) {
        ...
        heightWaterDifference = amplitude - amplitude * sin(spatialFrequency * sqrt(local_uv_coords.x * local_uv_coords.x + local_uv_coords.y * local_uv_coords.y) + timeFrequency * timeValue);

    }


	vec3 local_pos_3d = vec3(posX, fragHeight - heightWaterDifference, posY);
