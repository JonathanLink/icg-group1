...
void main() {
    vec3 normal = normalize(
        normal_matrix *
        vec4(cross(friend_pos_1 - my_pos, friend_pos_2 - my_pos), 1)
    ).xyz;
    vec3 light = normalize(vec3(0.0, -1.0, 0.0));
    float diffuse_coefficient = max(0.0, dot(normal, light));

    if (my_height <= 0.39) {
    	 color = vec3(0.1, 0.1, 1.0) * diffuse_coefficient;
    } else if (my_height <= 0.47) {
        color = vec3(1.0, 1.0, 0.1) * diffuse_coefficient;
    } else if (my_height <= 0.7) {
        color = vec3(0.1, 1.0, 0.1) * diffuse_coefficient;
    } else {
